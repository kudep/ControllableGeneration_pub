...
if os.path.isdir(data_db):
    ...
    pcmd = ' '.join(cmd) #% {'CODE_ROOT': CODE_ROOT}
    print(pcmd)
    ret = sp.run([PYTHON_EXE] + cmd, stdout=sp.PIPE, stderr=sp.STDOUT, cwd=PROJECT_FOLDER)
    if ret.returncode != 0:
...
parg = ' '.join(args)
ptrain_cmd = train_cmd + ' ' + parg
print(PYTHON_EXE + ' ' + ptrain_cmd)
logger.info('#########################################################################')
with open('./output.log', 'wb') as f: 
    process = sp.Popen([PYTHON_EXE, train_cmd] + args, stdout=sp.PIPE, stderr=sp.STDOUT, cwd=PROJECT_FOLDER)
