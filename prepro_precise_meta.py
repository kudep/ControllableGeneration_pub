...
def _get_file_len(corpus):
    n_line = int(sp.check_output(['wc', '-l', corpus],
...
def _get_inputs_from_text(text, tokenizer):
    srcs, tgt, *meta = text.strip().split('\t')
    weights = []
    inputs = []
    utterances = srcs.split(' EOS ')
    for src in utterances[:-1]:
        src_weight, src = _norm_text(src)
        context_id = tokenizer.encode(src)
        weights.append(src_weight)
        inputs.append(context_id)
    # last piece will be glue to target, so now
    # any part of target can have independent meta (no EOS btw srcs and tgt)
    src_weight, src = _norm_text(utterances[-1])
    tgt_weight, tgt = _norm_text(tgt)
    if tgt_weight != 0:  # always true for us
        response_id = tokenizer.encode(tgt)
        if src:
            starting_id = tokenizer.encode(src + ' ')
            weights.append([src_weight] * len(starting_id) +
                           [tgt_weight] * len(response_id))
            inputs.append(starting_id + response_id)
        else:
            weights.append(tgt_weight)
            inputs.append(response_id)
    return weights, inputs, meta


def _make_features(id_, weights, inputs, meta, tokenizer, max_len):
    ...
    for ids, w in zip(inputs, weights):
        if len(ids) > max_len:
            if len(sents) >= 2:
                feat = _make_feature(id_ + i, sents, ws, end_of_text_id, meta)
                if feat is not None:
...
def _make_feature(id_, sents, ws, eos, meta):
    if all(not isinstance(w, list) and w == 0 for w in ws[1:]):
        return None
    ...
    with open(args.corpus, "r", encoding="utf-8") as reader, \
        ...
        for line in tqdm(reader, total=_get_file_len(args.corpus)):
            try:
                ...
                weights, inputs, meta = _get_inputs_from_text(line, toker)
                if args.reverse:
                ...
                if len(weights) < 2:
                    continue
                features = _make_features(n_example, weights, inputs, meta,
