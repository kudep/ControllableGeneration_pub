...
class GPT2FeatureDataset(Dataset):
    ...
    def collate(features):
        ...
        return (input_ids, position_ids, token_type_ids, labels, meta)
...
class BucketingDataLoader(object):
    ...
    def __init__(self, db_name, batch_size, max_seq_len,
                 bucket=100, shuffle=True, reverse_meta=False):
        ...
        for key in keys:
            ...
            for feat in chunk:
                if feat['input_len'] > self.max_len:
                    continue
                if self.reverse_meta:
                    feat['meta'] = feat['meta'][::-1]
                trunc_chunk.append(feat)
                lens.append(feat['input_len'])

            feat_by_meta = {}
            for feat, flen in zip(trunc_chunk, lens):
                meta = tuple(feat['meta'])
                if meta not in feat_by_meta:
                    feat_by_meta[meta] = ([], [])
                feat_by_meta[meta][0].append(feat)
                feat_by_meta[meta][1].append(flen)

            for same_meta_chunks, same_meta_lens in feat_by_meta.values():
                dataset = GPT2FeatureDataset(same_meta_chunks, self.max_len)
                sampler = BucketSampler(same_meta_lens, self.bucket_size,
                                        self.batch_size,
...
class MixBucketingDataLoader(object):
    """ this mixes several data sources"""

    def __init__(self, db_names, batch_size, max_seq_len,
                 bucket=100, shuffle=True, num_sequential_batches=50):
        self.shuffle = shuffle
        self.num_sequential_batches = num_sequential_batches
        self.loaders = []
        for db_name in db_names:
            self.loaders.append(
                BucketingDataLoader(
                    db_name, batch_size, max_seq_len, bucket, shuffle))

    def __iter__(self):
        iterators = [(i, iter(loader)) for i, loader in
                     enumerate(self.loaders)]
        epoch_finished = set()
        while len(epoch_finished) != len(self.loaders):
            if self.shuffle:
                random.shuffle(iterators)
            active_iterators = []
            for iterator_id, iterator in iterators:
                try:
                    for _ in range(self.num_sequential_batches):
                        yield iterator_id, next(iterator)
                    active_iterators.append((iterator_id, iterator))
                except StopIteration:
                    epoch_finished.add(iterator_id)
                    active_iterators.append(
                        (iterator_id, iter(self.loaders[iterator_id])))
        ...
        if input_ids_len > max_seq_length:
            if len(context_id) > input_ids_len - max_seq_length:
                ...
                response_id = response_id[
                              :max_seq_length - len(context_id) - 2]

        input_ids = context_id + [end_of_text_id] + response_id + [
            end_of_text_id]

        # label simplely is next token in sequences. MASK all context_id tokens except for the last one
        lm_labels = [-1] * len(context_id) + response_id + [end_of_text_id] + [
            -1]
            ...
            with open(self.corpus, 'r', encoding="utf-8") as corpus:
                ...
                while True:
                    ...
                    while True:
                        ...
                        src, tgt, *meta = line.strip().split('\t')
                        src = src.replace(' EOS ', END_OF_TEXT_TOKEN)
                        if self.norm:
                            ...
                            tgt_line = tgt.strip()
                        examples.append(
                            ExampleWithMeta(i, src_line, tgt_line, meta),
                        )
                        i += 1
                        cur_bs += 1
                        if cur_bs >= self.bs:
                    ...
                    for feat in features:
                        ...
                        if meta not in feat_by_meta:
                            feat_by_meta[meta] = []
                        feat_by_meta[meta].append(feat)

                    for same_meta_features in feat_by_meta.values():
                        batch = self._batch_feature(same_meta_features)
                        yield batch
        except StopIteration:
            pass
